...
def send(tensor, dest_stage, async_op=False, fp32_comm=False):
    ...
    if async_op:
        global _async
        op = dist.isend(tensor_to_broadcast, dest_rank)
        ...
        if can_send_recv():
            dist.send(tensor_to_broadcast, dest_rank)
            if fp32_comm and tensor is not tensor_to_broadcast:
                tensor.copy_(tensor_to_broadcast)
        else:
...
def recv(tensor, src_stage, async_op=False, fp32_comm=False):
    ...
    if async_op:
        global _async
        op = dist.irecv(tensor_to_broadcast, src_rank)
        if fp32_comm and tensor is not tensor_to_broadcast:
            tensor.copy_(tensor_to_broadcast)
        _async.append(op)
    else:
        if can_send_recv():
            dist.recv(tensor_to_broadcast, src_rank)
            if fp32_comm and tensor is not tensor_to_broadcast:
                tensor.copy_(tensor_to_broadcast)
        else:
            group = _get_send_recv_group(src_stage, dest_stage)
