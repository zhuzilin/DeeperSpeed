...
from deepspeed.runtime.fp16.fused_optimizer import (
    FP16_Optimizer,
    FP16_FUSED_SUPPORTED_OPTIMIZERS,
    is_fp16_fused_supported_optimizer,
)
...
class DeepSpeedEngine(Module):
    ...
    def register_forward_hook(
        self, layers_to_hook: list, layer_name_pattern: str = "transformerlayer"
    ):
        ...
        def hook_fn(module, input, output):
            ...
            self.layer_outputs[key] = [
                o.cpu() if torch.is_tensor(o) else o for o in output
            ]
        ...
        def get_all_layers(net):
            for name, layer in net._modules.items():
                ...
                elif hasattr(layer, "__class__") and self.layer_name_pattern.search(
    ...
    def _configure_lr_scheduler(self, client_lr_scheduler):
        ...
        if lr_scheduler:
            ...
            if isinstance(client_lr_scheduler, Callable):
                if self.global_rank == 0:
                    logger.info(
                        "DeepSpeed using client callable to create LR scheduler"
                    )
    ...
    def _do_sanity_check(self):
        assert isinstance(
            self.client_optimizer, (type(None), Optimizer, Callable)
            ...
            if hasattr(p, "allreduce") and not p.allreduce:
                if torch.is_tensor(p) and is_replicated(p):
                    if (
                        ...
                        and self.allreduce_always_fp32()
                    ):
                        p.data = p.float().data
                    dist.broadcast(
                        p,
                ...
                if torch.is_tensor(p) and is_replicated(p):
                    dist.broadcast(
                        p, self.broadcast_src_rank, group=self.data_parallel_group
                    )
    ...
    def _configure_distributed_model(self, model):
        ...
        if not self.pipeline_parallelism:
            ...
            if self.mpu is not None:
                if groups.is_initialized():
                    ...
                    assert (
                        ...
                        == groups.get_model_parallel_world_size()
                    ), "mpu object provided must match mpu object provided to groups.initialize()"
                else:
                    # Scenario 3
            ...
            self.broadcast_src_rank = _get_global_rank(
                groups.get_data_parallel_group(), 0
            )
        else:
            ...
            self.broadcast_src_rank = _get_global_rank(
                self.mpu.get_data_parallel_group(), 0
            )
        ...
        if self.has_moe_layers:
            ...
            self.expert_broadcast_src_rank = _get_global_rank(
                groups.get_expert_data_parallel_group(), 0
            )
    ...
    def _configure_optimizer(self, client_optimizer, model_parameters):
        ...
        if self.global_rank == 0:
            logger.info(
                "DeepSpeed Basic Optimizer = {}".format(
    ...
    def _configure_fp16_optimizer(self, optimizer):
        ...
        if (
            isinstance(optimizer, fused_opts)
    ...
    def _take_model_step(self, lr_kwargs, block_eigenvalue={}):
        if self.gradient_clipping() > 0.0:
            ...
            if not (
                self.fp16_enabled() or self.amp_enabled() or self.zero_optimization()
            ):
        ...
        if self.store_gradients:
            if self.store_gradients_cpu:
                self.stored_gradients = list(
                    [p.grad.clone().cpu() for p in self.module.parameters()]
                )
            else:
                self.stored_gradients = list(
                    [p.grad.clone() for p in self.module.parameters()]
                )
    ...
    def step(self, lr_kwargs=None):
        ...
        if self.wall_clock_breakdown():
            ...
            self.timers.log(
                names=timer_names, reset=False, memory_breakdown=self.memory_breakdown()
            )
                    ...
                    if self.global_rank == 0:
                        self.summary_events = [
                            ...
                            (
                                ...
                                self.timers("backward_inner").elapsed(reset=False)
                                * 1000.0,
                                self.global_samples,
                            ),
                            (
    ...
    def buffered_allreduce_fallback(self, grads=None, elements_per_buffer=500000000):
        ...
        if self.has_moe_layers:
            ...
            for i, bucket_tuple in enumerate(expert_split_buckets):
                bucket_type, bucket = bucket_tuple
                if bucket_type == CSRTensor.type():
                    self.csr_allreduce_no_retain(
                        bucket, groups.get_expert_data_parallel_group()
                    )
                else:
    ...
    def csr_all_gather(self, value, dp_group):
        ...
        if value.dim() == 1:
            ...
            tensor_list = [
                value.new_zeros(max_size, value.size()[1])
                for _ in range(dist.get_world_size(group=dp_group))
            ]
        ...
        for dev_idx, t in enumerate(tensor_list):
            ...
            tensors.append(
                t.index_select(0, torch.LongTensor(range(size)).to(self.device))
            )
    ...
    def _get_mp_rank_zero_checkpoint_names(self, load_dir, tag, mp_rank, dp_world_size):
        ...
        for dp_rank in range(dp_world_size):
            ckpt_name = self._get_rank_zero_ckpt_name(
                checkpoints_path=load_dir, tag=tag, mp_rank=mp_rank, dp_rank=dp_rank
            )
            zero_ckpt_names.append(ckpt_name)

        return zero_ckpt_names

    def _get_all_zero_checkpoint_names(
        self, load_dir, tag, mp_world_size, dp_world_size
    ):
        zero_ckpt_names = []
        for mp_rank in range(mp_world_size):
            mp_rank_ckpt_names = self._get_mp_rank_zero_checkpoint_names(
                load_dir=load_dir, tag=tag, mp_rank=mp_rank, dp_world_size=dp_world_size
            )
    ...
    def _save_moe_checkpoint(self, save_dir, tag, client_state={}):
        ...
        for global_expert_id, expert_state_dict in experts_state_dict.items():
            expert_save_dir = self._get_expert_ckpt_name(
                save_dir, global_expert_id, tag
            )
            logger.info(
        ...
        torch.save(
            optimizer_state, self._get_optimizer_ckpt_name(save_dir, tag, expp_rank)
        )
